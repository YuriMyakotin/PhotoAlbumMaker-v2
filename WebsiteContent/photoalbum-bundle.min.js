 ...
 * @license GPLv3
 */
function MakeNavStr(){var n=CurrentFolderID,t;if(n===0){window.NavStr.innerHTML=SiteName;return}for(t=Folders.get(n),window.NavStr.innerHTML=t.N;;){if(n=t.PI,n===0){window.NavStr.innerHTML=`<a href="#" onclick="LoadAlbum(0)">${SiteName}</a>&nbsp;>> ${window.NavStr.innerHTML}`;return}t=Folders.get(n);window.NavStr.innerHTML=`<a href="#" onclick="LoadAlbum(${n})">${t.N}</a>&nbsp;>> ${window.NavStr.innerHTML}`}}function LoadAlbum(n){CurrentFolderID=n;MakeNavStr();var t="";if(n===0)document.title=SiteName,t=SiteDescription;else{const i=Folders.get(n);document.title=`${SiteName}: ${i.N}`;i.C!=null&&(t=i.C)}window.FolderDesc.innerText=t;window.FolderDesc.style.display=t.length>0?"block":"none";LoadSubfolders();isPrivate&&window.addEventListener("contextmenu",n=>n.preventDefault())}function LoadSubfolders(){while(window.VideosLst.lastChild)window.VideosLst.removeChild(window.VideosLst.lastChild);while(window.ImgLst.lastChild)window.ImgLst.removeChild(window.ImgLst.lastChild);while(window.FoldersLst.lastChild)window.FoldersLst.removeChild(window.FoldersLst.lastChild);let f=CurrentFolderID===0?RootFolders:Folders.get(CurrentFolderID).SubFolders,i=f.length,r=CurrentFolderID,n=document.getElementById("FoldersLst");if(i>0){window.FoldersLst.style.display="block";f.forEach(t=>{const u=Folders.get(t),o=`/${u.TF.toString()}/thumbs/${u.TN}`,i=document.createElement("a"),s=colors[r%colors.length];r++;i.style.width=(ThumbSize*2).toString()+"px";i.style.height=(ThumbSize+8).toString()+"px";i.style.backgroundColor=s;i.className="Fol";i.onclick=function(){LoadAlbum(u.I)};i.id=`f${u.I.toString()}`;i.title=u.D!=null?u.D:"";i.href="#";const f=document.createElement("img");f.className="F";f.src=o;i.appendChild(f);const e=document.createElement("span");e.className="folder-name";e.textContent=u.N;i.appendChild(e);n.appendChild(i)});let t=new Masonry(n,{itemSelector:".Fol",isFitWidth:!0,transitionDuration:0})}else window.FoldersLst.style.display="none";if(CurrentFolderID===0){window.VideosLst.style.display="none";window.ImgLst.style.display="none";return}let u=Folders.get(CurrentFolderID);if(i=u.V.length,n=document.getElementById("VideosLst"),i>0){window.VideosLst.style.display="block";for(let t=0;t<i;t++){let f=u.V[t],i=document.createElement("a"),s=colors[r%colors.length];r++;i.className="Vid";i.title=f.D;i.id=`Vid${f.N}`;i.href="#";i.style.width=(ThumbSize*2.2).toString()+"px";i.style.height=(ThumbSize*.8+4).toString()+"px";i.style.backgroundColor=s;i.style.wordWrap="normal";i.onclick=function(){ShowVideo(f.N,f.D)};let e=document.createElement("img");e.className="V";e.src="/Videos/thumb_"+f.N+".jpg";e.style.height=(ThumbSize*.8).toString()+"px";e.onerror=function(){this.src="/Content/Video.png"};i.appendChild(e);let o=document.createElement("span");o.className="video-name";o.textContent=f.D;i.appendChild(o);n.appendChild(i)}let t=new Masonry(n,{itemSelector:".Vid",isFitWidth:!0,transitionDuration:0})}else window.VideosLst.style.display="none";if(n=document.getElementById("ImgLst"),i=u.Im.length,i>0){window.ImgLst.style.display="block";for(let r=0;r<i;r++){var t=u.Im[r];let i=document.createElement("A");i.className="Img";i.href=CurrentFolderID.toString()+"/slides/"+t.N;i.rel="group";i.title=t.N;i.style.width=ThumbSize.toString()+"px";i.style.height=ThumbSize.toString()+"px";let e=document.createElement("Img");e.className="I";e.src=CurrentFolderID.toString()+"/thumbs/"+t.N;let f="<div>";t.C!=null&&(f+=`<span class='cmnt'>${t.C}</span><br />`);f+=`<a class='OrigImgLink' id='OrigImg' href='#' title='Original image' onclick="ShowOriginalImage(${CurrentFolderID},'${t.N}');return false;">${t.N}</a><br />`;f+=ExifInfoToStr(t)+"<br />"+GPSInfoToStr(t);f+="<br /><\/div>";i.setAttribute("data-sub-html",f);i.appendChild(e);n.appendChild(i)}let r=new Masonry(n,{itemSelector:".Img",isFitWidth:!0,transitionDuration:0});ImgLightGallery=lightGallery(n,{licenseKey:L,mode:"lg-fade",mousewheel:!0,download:!1,speed:0,backdropDuration:0,thumbnail:!0,slideEndAnimation:!1,startAnimationDuration:0,getCaptionFromTitleOrAlt:!1,subHtmlSelectorRelative:!0,startClass:"",toogleThumb:!1,plugins:[lgThumbnail,lgAutoplay,lgRelativeCaption]})}else window.ImgLst.style.display="none"}function ShowOriginalImage(n,t){var i=ImgLightGallery.index;ImgLightGallery.closeGallery();const r=lightGallery(document.getElementById("OrigImg"),{licenseKey:L,startAnimationDuration:0,backdropDuration:0,counter:!1,download:!isPrivate,speed:0,dynamic:!0,dynamicEl:[{src:n.toString()+"/"+t}],plugins:[lgZoom]});r.openGallery(0);document.getElementById("OrigImg").addEventListener("lgAfterClose",()=>{ImgLightGallery.openGallery(i)})}function ShowVideo(n,t){const i=isPrivate?"nodownload":"",r=lightGallery(document.getElementById(`Vid${n}`),{licenseKey:L,autoplayVideoOnSlide:!0,startAnimationDuration:0,backdropDuration:0,counter:!1,download:!1,speed:0,dynamic:!0,dynamicEl:[{video:{source:[{src:`/videos/${n}`,type:"video/mp4"}],attributes:{preload:!1,controls:!0,controlsList:i}},subHtml:`<h2>${t}</h2>`}],plugins:[lgVideo]});r.openGallery(0)}function ConvertDEGToDMS(n,t){const i=parseFloat(n),r=Math.abs(i),u=Math.floor(r),f=(r-u)*60,e=Math.floor(f),s=((f-e)*60).toFixed(2);var o;return o=t?i>=0?"N":"S":i>=0?"E":"W",u+"°"+e+"'"+s+'"'+o}function ExifInfoToStr(n){var t="";return n.D!=null&&(t+=n.D+"&nbsp; "),n.Cn!=null&&(t+=n.Cn+"&nbsp; "),n.F!=null&&(t+=n.F+"mm&nbsp; "),n.I!=null&&(t+=`ISO:${n.I}&nbsp; `),n.E!=null&&(t+=n.E+"s&nbsp; "),n.A!=null&&(t+=`f/${n.A}&nbsp; `),t}function GPSInfoToStr(n){var t="";if(n.La!=null&&n.Lo!=null){const i=n.La+","+n.Lo;let r=ConvertDEGToDMS(n.La,!0)+",&nbsp;"+ConvertDEGToDMS(n.Lo,!1);n.Al!=null&&(r+=`,&nbsp${n.Al}`);t=`<a href="https://www.google.com/maps/dir//${i}/@${i},18z" target="_blank">${r}</a>`}return t}(function(n,t){typeof exports=="object"&&typeof module!="undefined"?module.exports=t():typeof define=="function"&&define.amd?define(t):(n=typeof globalThis!="undefined"?globalThis:n||self,n.lightGallery=t())})(this,function(){"use strict";
/*! *****************************************************************************
 ...
 * @license GPLv3
 */
const colors=["#F3B200","#77B900","#AD103C","#2572EB","#632F00","#B01E00","#7200AC","#006AC1","#008287","#56C5FF","#4617B4","#C1004F","#00C13F","#FF981D","#FF2E12","#199900","#FF1D77","#AA40FF","#1FAEFF","#00D8CC","#91D100","#E1B700","#FF76BC","#00A3A3","#FE7C22","#647687"];let CurrentFolderID=0;const Folders=new Map;let ThumbSize=0,ImgLightGallery,SiteName="",SiteDescription="",isPrivate=!1,Version=0,RootFolders=[];const L=atob("RkEyMzEzQkItQ0UzMTQ5Q0YtQUJDOUUxNEYtMTVCRkU2RkI=");document.addEventListener("DOMContentLoaded",async function(){const t=await fetch("Site.json"),n=await t.json();n.D!=null&&(SiteDescription=n.D);n.P!=null&&(isPrivate=n.P);n.V!=null&&(Version=n.V);ThumbSize=n.TS;SiteName=n.N;window.sitestats.innerHTML=n.TIC.toString()+" images, "+n.TVC.toString()+" videos, last updated "+n.LC;window.siteversion.innerHTML=(Version/100).toFixed(2).toString();const i=n.F;i.forEach(n=>{n.SubFolders=[],Folders.set(n.I,n)});Folders.forEach(n=>{n.PI===0?RootFolders.push(n.I):Folders.get(n.PI).SubFolders.push(n.I)});LoadAlbum(0)});